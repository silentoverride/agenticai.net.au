# Raw Sources

Registry of every source. In-project paths are relative to project root.

## docs

- **Cloudflare Documentation** — `https://developers.cloudflare.com/` — collected 2026-05-02 → [Cloudflare infrastructure](../integrations/cloudflare.md)
- **Presenton Documentation** — `https://docs.presenton.ai/` — collected 2026-05-02 → [Presenton report generator](../reports/presenton.md)
- **Stripe Documentation** — `https://docs.stripe.com/` — collected 2026-05-02 → [Stripe platform docs](../integrations/stripe-platform.md)
- **Retell AI — Introduction** — `https://docs.retellai.com/general/introduction` — collected 2026-05-02 → [Retell platform overview](../integrations/retell.md)
- **AI Business Assessment Business Plan** — `docs/AI Business Assessment Business Plan.md` — collected 2026-05-02 → [Service model](../business/service-model.md), [Question knowledgebase](../agents/question-knowledgebase.md)
- **Question Knowledgebase** — `docs/question-knowledgebase.md` — collected 2026-05-02 → [Question knowledgebase](../agents/question-knowledgebase.md)
- **Retell Chat Agent Workflow** — `docs/retell-annie-chat-agent-workflow.md` — collected 2026-05-02 → [Annie chat agent](../agents/annie-chat-agent.md)
- **Retell Voice Agent Workflow** — `docs/retell-annie-voice-agent-workflow.md` — collected 2026-05-02 → [Annie voice agent](../agents/annie-voice-agent.md)
- **Retell Report Agent Handoff** — `docs/retell-report-agent-handoff.md` — collected 2026-05-02 → [Report pipeline](../operations/report-pipeline.md)
- **Twilio and Retell Setup** — `docs/twilio-retell-setup.md` — collected 2026-05-02 → [Twilio integration](../integrations/twilio.md)
- **Stripe Setup** — `docs/stripe-setup.md` — collected 2026-05-02 → [Stripe payments](../integrations/stripe.md)
- **Connect Twilio Number** — `docs/connect-twilio-number-to-retell-voice-agent.md` — collected 2026-05-02 → [Twilio integration](../integrations/twilio.md)
- **Voice Agent Script** — `docs/voice-agent-script.md` — collected 2026-05-02 → [Annie voice agent](../agents/annie-voice-agent.md)
- **Voice Agent Disclaimer** — `docs/voice-agent-disclaimer.md` — collected 2026-05-02 → [Annie voice agent](../agents/annie-voice-agent.md)
- **Voice Agent User Test Script** — `docs/retell-voice-agent-user-test-script.md` — collected 2026-05-02 → [Annie voice agent](../agents/annie-voice-agent.md)
- **Business Plan (docx)** — `docs/AI Business Assessment Business Plan.docx` — collected 2026-05-02
- **Business Plan (pdf)** — `docs/AI Business Assessment Business Plan.pdf` — collected 2026-05-02
- **Assessment Deck (pptx)** — `docs/ai-tools-assessment-redacted.pptx` — collected 2026-05-02
- **Verification Deck (pptx)** — `docs/Deck-Verification-Services-AI-Business-Assessment_aa3a9a2d-e8b1-4a13-9cd8-eca3021bfad8.pptx` — collected 2026-05-02

## conversations

- **Annie — AI Business Assessment Chat** — `raw-sources/conversations/2026-05-01-annie-business-assessment-chat.json` — collected 2026-05-02 → [Annie chat agent](../agents/annie-chat-agent.md)

## configs

- **Annie Voice Agent Config** — `docs/annie_voice_agent.json` — collected 2026-05-02 → [Annie voice agent](../agents/annie-voice-agent.md)
- **Question Knowledgebase (JSON)** — `docs/question-knowledgebase.json` — collected 2026-05-02 → [Question knowledgebase](../agents/question-knowledgebase.md)

## decks

_(No entries yet)_

## figures

_(No entries yet)_
