...
describe('Role-Based Access', () => {
  ...
  it('allows users with staff role', async () => {
    await expect(requireStaff(mockLocals('user_1'), mockDb('staff'))).resolves.toBe('staff');
  });

  it('allows users with admin role', async () => {
    await expect(requireStaff(mockLocals('user_1'), mockDb('admin'))).resolves.toBe('admin');
  });

  it('rejects authenticated non-staff', async () => {
    await expect(requireStaff(mockLocals('user_1'), mockDb('client'))).rejects.toMatchObject({ status: 403 });
  });

  it('rejects unauthenticated users', async () => {
    await expect(requireStaff(mockLocals(null), mockDb('staff'))).rejects.toMatchObject({ status: 401 });
  });
});
...
describe('NFR Compliance', () => {
  ...
  it('load more fetches next page with compound cursor', () => {
    ...
    const url = `/api/staff/gates?cursor=${encodeURIComponent(cursor)}&limit=50`;
