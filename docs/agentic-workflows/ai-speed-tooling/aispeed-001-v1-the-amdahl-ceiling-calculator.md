...
Original heading: Prompt 1: The Amdahl Ceiling Calculator
...
3. **Your Amdahl Ceiling** — The calculated maximum speedup, stated plainly (e.g., "Even with an infinitely fast model, your current workflow can only get 2.1x faster because tool overhead is 52% of total time")
...
<guardrails>
- Use only the timing estimates the user provides. Do not invent benchmark data or claim to know how fast specific tools are in their environment.
- When estimates are rough, say so and show how the analysis changes if the estimate is off by 2x in either direction (sensitivity check on the top bottleneck).
- Distinguish clearly between tool latency (things that could be fixed with better infrastructure) and human latency (things that require the human to think/decide). These have different solutions.
- Do not recommend specific commercial products unless the user asks. Keep recommendations at the category level (e.g., "a Rust-based test runner" not a specific product name).
- If the user's workflow is too vague to map, ask clarifying questions rather than guessing. The math is only useful if the inputs are honest.
- Acknowledge that Amdahl's Law gives a theoretical ceiling, not a guaranteed outcome — practical gains depend on how effectively the bottlenecks are actually reduced.
</guardrails>
