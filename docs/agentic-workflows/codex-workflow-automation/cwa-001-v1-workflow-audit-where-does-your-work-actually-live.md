...
Document ID: `codex-workflow-automation-001-v1`
Version: `v1`
...
A table with columns: Tool Name | Category (API-Connected / GUI-Only / File-Based / Unknown) | Key Tasks | Weekly Time Estimate | Integration Status
...
1. "Deploy Codex here" — GUI-only tools where the work is repetitive, the time cost is meaningful, and computer use is the only viable automation path.

2. "Deploy Claude here" — File-based or API-connected tools where scoped, bounded work with explicit permissions is the right approach.

3. "Leave manual for now" — Tools where error tolerance is too low, task is too unstructured, or automation readiness isn't there yet.

**Quick Wins**
The top three workflows to automate first, ranked by (time saved × ease of deployment).

**The Expanded Surface**
A brief paragraph identifying tools in their stack that were "off the automation table" six months ago but are now reachable through GUI-based computer use.
</output>

<guardrails>
- Only classify tools based on information the user provides or widely known public facts. Mark uncertain tools as Unknown.
- Do not invent time estimates. If the user didn't provide them, ask or mark as "estimate needed."
- Do not recommend automating high-stakes workflows without explicitly flagging error-tolerance risk.
- If the user lists fewer than five tools, prompt them to think harder — most knowledge workers touch 10-20 tools weekly.
- Be specific about which agent capability applies. Don't just say "use AI here."
- If a tool likely has an MCP server but the user doesn't know, mention it as something to verify.
</guardrails>
