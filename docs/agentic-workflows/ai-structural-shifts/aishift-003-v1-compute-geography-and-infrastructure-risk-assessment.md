...
Original heading: Prompt 3: Compute Geography & Infrastructure Risk Assessment
...
2. For each location or region the user is considering (or currently using), assess four risk dimensions:
   ...
   **Permitting & Local Politics Risk**
   ...
   - What's the local political climate toward data centers? (Reference: national net approval is just +2%, lower than gas plants or nuclear facilities)
   ...
   **Geopolitical & Physical Security Risk**
   - Is the region exposed to kinetic threats? (Reference: Iranian drone strikes on AWS facilities in UAE/Bahrain in March 2026)
...
3. Produce a constraint map: a clear visualization (table format) showing which locations face which bottlenecks, with a severity rating (Low / Medium / High / Blocking) for each dimension.

4. Recommend a deployment strategy:
...
5. Build a contingency playbook:
   - For each primary location: what's the specific disruption scenario, what triggers the contingency, and what's the migration path?
   - Flag where data residency rules would prevent rapid failover (as happened with the UAE insurance platform after the AWS strikes)
   - Estimate the time and cost to execute each contingency

6. Close with a 12-month outlook: which of their current or planned locations is likely to become more constrained, and which is likely to open up?
</instructions>
...
- If the user's scale is small enough that geography risk is genuinely low (e.g., a small app on a major cloud provider with automatic failover), say so rather than manufacturing urgency.
