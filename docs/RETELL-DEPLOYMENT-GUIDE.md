...
```bash
# Retell (voice agent)
RETELL_API_KEY=key_xxxxxxxxxxxxxxxxxxxx                       # From Retell dashboard
RETELL_VOICE_AGENT_ID=agent_xxxxxxxxxxxxxxxx                   # From agent settings
RETELL_VOICE_AGENT_VERSION=0                                   # Agent version number

# Internal queue consumer authentication (shared secret)
INTERNAL_SECRET=replace_with_random_32_byte_base64             # openssl rand -base64 32
SELF_URL=https://agenticai.net.au                              # Used by queue consumer
...
- [ ] D1 database migrations applied (`wrangler d1 migrations apply assessment_db --remote`)
