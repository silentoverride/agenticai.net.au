...
export async function isEventProcessed(eventId: string): Promise<boolean> {
  return withDb<boolean>('isEventProcessed', false, async db => {
...
export async function markEventProcessed(eventId: string, eventType: string) {
  return withDb('markEventProcessed', undefined, async db => {
    await db.exec(
...
export async function pruneProcessedEvents(olderThanDays = 30) {
  return withDb('pruneProcessedEvents', undefined, async db => {
